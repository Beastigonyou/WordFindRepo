...
function printPuzzle(){
    ...
    global $puzzle, $currentWord, $keyPuzzle, $boardData;
    //print puzzle itself
...
function addWord($theWord, $dir){
    ...
    switch ($dir){
        ...
        case "SW":
            ...
            // row from 0 to board height - length of word
            $newCol = rand(strlen($theWord),$boardData["width"]- 1);
            $newRow = rand(0,$boardData["height"]- strlen($theWord)-1);
...
function fillBoard(){
    ...
    global $currentWord;
    $direction = array("N", "S", "E", "W","NE","NW","SE","SW");
    $itWorked = TRUE;
    $counter = 0;
    $keepGoing = TRUE;
    while($keepGoing){
        $dir = rand(0, 7);
        $result = addWord($currentWord[$counter], $direction[$dir]);
        if ($result == FALSE){
